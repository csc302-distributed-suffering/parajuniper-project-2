...
This was important to do first since it would determine the patterns that we are using in our backend code. If this was not done first we would have to make these changes once the backend code was complete, which would make them far more difficult and time consuming.
  ...
  Convert Backend of Project to use TS - Nikita & Viktar - Complete - Due:  31/10/2021
...
Backend Routing & Testing:
 
Connect our server to FHIR through FHIRKitClient - Nikita & Viktar - Complete - Due: 04/10/2021
   1. Add instance of FHIRKitClient
   2. Choose FHIR server to connect to
...
Decide on what data will be retrieved by the queries added in this milestone - Decision - Nikita & Viktar - Complete - Due: 31/10/2021
   1. FHIR returns a lot of data. We need to decide what we are keeping and what we are dropping.
...
Decide on what parameters will be used by our queries - Decision - Nikita & Viktar - Complete - Due: 31/10/2021
   1. FHIR queries contain a lot of parameters and we need to decide on which of them will be supported by the routes implemented in this milestone.
...
Decide on client-side vs. server-side paging for database queries that can potentially return a large number of records - Decision - Nikita & Viktar - Due: 01/11/2021
   1. FHIR paging support is unique in that it does not support a page parameter and only accepts a count of records per query. Instead of a page parameter, it returns a URL to the next page in the case that the total number of results exceeded the count provided.
...
We decided on a client keeping track of this information and sending a request to our server with the link to the next page provided by FHIR to our server which would query the data for the UI.
        ...
        The reasons for this back and forth is:
...
Add route to retrieve a list of patients that match a <First Name, Last Name> tuple - Nikita & Viktar - Complete - Due: 03/11/2021
   1. Add route for querying https://r4.smarthealthit.org/Patient?name={}&family={}&count={}
   2. Parse our query parameters into FHIR query parameters.
   3. Perform server side error checking (integrity of request to FHIR and the response from FHIR) and return the appropriate status code to UI.
   4. Return the total number of matches as well as the next page link in our response so that we may page the data on the UI.
...
Add route to retrieve a single patient by their ID - Nikita & Viktar - Complete - Due: 03/11/2021
   1. Add route for querying https://r4.smarthealthit.org/Patient?id={}
   2. Parse our query parameters into FHIR query parameters.
   3. Perform server side error checking (integrity of request to FHIR and the response from FHIR) and return the appropriate status code to UI.
...
Frontend:
Decide on what pages our UI is going to have when complete - Decision - Everyone - Complete - Due: 17/10/2021
   1. What pages are necessary for our application and what data should each page contain?
...
This was the highest priority task for the UI. We needed to have a concrete understanding and idea of what the UI will look like so that it can be implemented efficiently.
 ...
 Decision is made.
  ...
  Decide on display format for data - Decision - Everyone - Complete - Due: 17/10/2021
   1. Decide on how exactly patient records will be displayed.
   ...
   Priority Rationale:
   In addition to the above, before development could start, we needed to decide how patient   data will be displayed. We were planning on having a mockup of the UI being done
   and therefore, we needed to decide on this before we started working on it.

   Acceptance Criteria:
   Decision is made.

Rough mockup of the UI - Feature - Felicia - Complete - Due: 24/10/2021
   1. Mockup of the Log In and Patient Search pages in Figma.
   2. Mockup for patient search results view.
   3. Mockup for single patient data display.
...
Acceptance Criteria:
The mockup contains the required pages and demonstrates the expected functionality.

Dummy UI, not attached to backend made using the mockup - Feature - Eric - Complete - Due: 31/10/2021
   1. Implementation of the Figma mockup (except for the log in page)
...
Acceptance Criteria:
The UI is implemented according to the mockup provided. (Excludes the log in page)

Attach the two backend routes to the UI - Feature - Viktar - Complete - Due: 03/11/2021
   1. Provide actions for making requests on our server in the UI code for the following routes:
      - /patients/list
      - /patients/info

   2. Add ability to search for patient by first and last name using the action for /patients/list
...
Acceptance Criteria:
User is able to enter the first and last names of a patient and get a list of matching patients back.
Search by ID is attached to the UI (without display).

Organize patient cards in a grid (or some other layout as long as we can see them) - Feature - Felicia - Not Started - Due: 04/11/2021
   1. Create a view to list all patient search results.
   2. Each patient returned by the query should get their own card that when clicked will display their detailed information.
   3. Each card should have the basic identifying information for a patient (first name, family name, id, age).
...
Acceptance Criteria:
User is able to see all patients returned by the query once they run the search.
   

Create patient cards for each patient returned by the backend routes and display patient-specific info in each of them - Feature - Felicia - In Progress - Due: 04/11/2021
   1. Create a patient card view that would hold all of the information returned per patient.
   2. For now this will just display the raw JSON. Parsing data and organizing it on the frontend will be done in milestone 3.
...
Acceptance Criteria:
User is able to click a patient card and see detailed information about that patient.
...
We need to have validation on a technical level and therefore, once development is complete, we should create tests for the code. This has lower priority as development has to be at least partially complete before testing can begin.
 ...
 Add backend unit tests for routes - Nikita - Complete - Due: 03/11/2021
   1. Test the routes.ts file to achieve >=85% coverage

 Acceptance Criteria:
Add tests for both new routes.
Verify edge cases are covered.
Ensure test run can be automated.

Documentation:
Although important, this is last since our codebase is currently small with few maintainers. Therefore, having complete documentation immediately is less important for the milestone.
Nevertheless, it is important to include it in the milestone, even if it is the item with the lowest priority.

 Add short function headers - Feature - Everyone - Not Done - Due: 04/11/2021
